 ...
 * 指令内容初始化
 */
func initMenu(lt *LinkTable,this *MainController) {
	menu,err := models.GerAllCmds()
	if err != nil {
		beego.Error(err)
	}
	for i:=0;i<len(menu);i++ {
		var tempDataNode Node
		tempDataNode.Cmd = menu[i].Cmd
		tempDataNode.Desc = menu[i].Desc
		//tempDataNode.Handler = func(menu[i].Handler)
	}
...
//自定义模板函数，序号加1
func IndexAddOne(index int) (result int) {
	result = index + 1
	return
}
